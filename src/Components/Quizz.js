...
<h2>Quiz Fun</h2>
<h4>Challenge  your Knowledge</h4>
</div>
        </div>
...
<div class="card-m">
  ...
  <div className="card-body-qs">
    ...
    <div className="question-section">
      ...
      <h2 className='h2-text'>Question {currentQuestion + 1} of {quizzes.length}</h2>
      <h5 className='h5-text'>Q.{quizzes[currentQuestion].questionText}</h5>
    </div>
    </div>
    </div>
...
return  <button  key={i} type="button" disabled={clicked} onClick={()=>handleAnswerOptions(ans.isCorrect)} className={`btn-o btn-warning mx-3 my-3 ${clicked & ans.isCorrect? "correct" : "btn"}`}>{ans.answerText}</button>
    })}
